...
public partial class Bill_Deletion : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //        string[] argsval = { cid};
            //        DataSet ds_lr = new DataSet();
            //        ds_lr = con.Sql_GetData("Bizconnect_Get_Details_LRNumber", args, argsval);
                    
            //        if (ds_lr.Tables[0].Rows.Count > 0)
            //        {
            //            ddl_LrNo.DataSource = ds_lr;
    ...
    //        string connectionString = ConfigurationManager.ConnectionStrings["Bizcon"].ConnectionString;
    //        sqlconn = new SqlConnection(connectionString);

    //        SqlDataAdapter sda = new SqlDataAdapter();
    //        SqlCommand sqlcmd = new SqlCommand("Bizconnect_GetLrNumberandConfirmnoByLrno", sqlconn);
    //        sqlcmd.CommandType = CommandType.StoredProcedure;
    //        sqlcmd.Parameters.AddWithValue("@ClientID", cid);
    //        sqlcmd.Parameters.AddWithValue("@LRNumber", LRNumber);
    ...
    protected void ddl_LrNo_SelectedIndexChanged(object sender, EventArgs e)
    {
           ...
           try
        {
            ...
            //dt = obj_class.Bizconnect_GetLrNumberandConfirmnoByLrno(Convert.ToInt32(Session["ClientID"].ToString()), ddl_LrNo.SelectedItem.Text);
             string cid = Session["ClientID"].ToString();
            string confirmno=txt_lrnumber.Text;

            string[] argslrnumber = { "@clientid", "@lrnumber" };
            string[] argsvallrnumber = { cid, confirmno };
            DataSet ds = new DataSet();
            ds = con.Sql_GetData("Bizconnect_Get_Details_for_LRNumber", argslrnumber, argsvallrnumber);
            if(ds.Tables[0].Rows.Count > 0)
            {
                ddl_ConfirmNo.DataSource = ds;
    ...
    protected void btn_Edit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if ((Convert.ToInt32(Session["UserID"]) == 381 && Session["name"].ToString() == "BABUTHOMAS BABUTHOMAS") || (Convert.ToInt32(Session["UserID"]) == 2921 && Session["name"].ToString() == "Amit Yadav") || (Convert.ToInt32(Session["UserID"]) == 3205 && Session["name"].ToString() == "Sandeep Tembhurkar"))
            {
    ...
    protected void txt_lrnumber_TextChanged(object sender, EventArgs e)
    {
        try
        {
            string cid = Session["ClientID"].ToString();
