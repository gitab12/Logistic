...
public partial class CNAmendment : System.Web.UI.Page
{
    ...
    if (Session["UserID"] != string.Empty && Convert.ToInt32(Session["UserID"].ToString()) > 0)
        ...
        if (!IsPostBack)
        {
           ...
           string pjtno = Session["pjtno"].ToString();
            ...
            ds_cn = obj_class.Get_CnAmendment(Session["pjtno"].ToString ());
            grd_CnAmendment.DataSource = ds_cn;
            grd_CnAmendment.DataBind();
        }
        }
        else
        {
    ...
    {
        ds_WBSNo = obj_class.Get_LoadWBSNoForCNAmendment(ddl_ProjectNo.SelectedValue.ToString());
     ...
     protected void btn_Search_Click(object sender, EventArgs e)
    {
        ...
        {
            ds_CNote = obj_class.Bizconnect_ThermaxCNote_SearchByProjectNoAndWBSNo(ddl_ProjectNo.SelectedValue.ToString(), ddl_WBSNo.SelectedValue.ToString(), "CNAmendment");
            grd_CnAmendment.DataSource = ds_CNote;
            grd_CnAmendment.DataBind();
        }
        else
        {
            ds_CNote = obj_class.Bizconnect_ThermaxCNote_SearchByProjectNoAndWBSNo(ddl_ProjectNo.SelectedValue.ToString(), string.Empty, "CNAmendment");
            grd_CnAmendment.DataSource = ds_CNote;
            grd_CnAmendment.DataBind();
...
protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        ExportGrid(grd_CnAmendment, "Report.xls");
