...
public partial class ImportExport_BuyerMaster : System.Web.UI.Page
{
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            obj_Class.BuyerName = txt_Buyername.Text;
            ...
            resp = obj_Class.Bizconnect_InsertImporExportBuyerDetails();
            if (resp != 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "<script>alert('Buyer details submitted successfully');</script>");
                ClearFields();
            }
            else
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "<script>alert('This buyer details already submitted ');</script>");
            }
        }
        catch (Exception ex)
        {
