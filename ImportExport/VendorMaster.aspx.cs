...
public partial class ImportExport_VendorMaster : System.Web.UI.Page
{
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            obj_Class.VendorName = txt_Vendorname.Text;
            ...
            resp = obj_Class.Bizconnect_InsertImportExportVendorDetails();
            if (resp != 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "<script>alert('Vendor details submitted successfully');</script>");
                ClearFields();
            }
            else
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "<script>alert('This vendor details alread submitted');</script>");
            }
        }
        catch (Exception ex)
        {
