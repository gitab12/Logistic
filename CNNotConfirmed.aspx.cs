...
using System.Data;
public partial class CNNotConfirmed : System.Web.UI.Page
{
     ...
     if (Session["UserID"] != string.Empty && Convert.ToInt32(Session["UserID"].ToString()) > 0)
        ...
        if (!IsPostBack)
        {
            ...
            string pjtno = Session["pjtno"].ToString ();
                ...
                {
                    //ds_cn = obj_class.Get_CnNotConfirmed(Session["pjtno"].ToString());
                    Session["pjtno"] = string.Empty;
                }
                ds_cn = obj_class.Get_CnNotConfirmed(Session["pjtno"].ToString());
            grd_Cnnotconfirmed.DataSource = ds_cn;
            grd_Cnnotconfirmed .DataBind();
        }
        }
        else
        {
    ...
    {
        ds_WBSNo = obj_class.Get_LoadWBSNoForCNNotConfirmed(ddl_ProjectNo.SelectedValue.ToString());
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        ...
        {
            ds_CNote = obj_class.Bizconnect_ThermaxCNote_SearchByProjectNoAndWBSNo(ddl_ProjectNo.SelectedValue.ToString(), ddl_WBSNo.SelectedValue.ToString(), "CNNotConfirmed");
            grd_Cnnotconfirmed.DataSource = ds_CNote;
            grd_Cnnotconfirmed.DataBind();
        }
        else
        {
            ds_CNote = obj_class.Bizconnect_ThermaxCNote_SearchByProjectNoAndWBSNo(ddl_ProjectNo.SelectedValue.ToString(), string.Empty, "CNNotConfirmed");
            grd_Cnnotconfirmed.DataSource = ds_CNote;
            grd_Cnnotconfirmed.DataBind();
...
protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        ExportGrid(grd_Cnnotconfirmed, "Report.xls");
